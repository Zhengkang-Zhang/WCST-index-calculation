...
filename <- "stefan_wcst_long_exp_fixed_64.csv"
stefan_wcst_long_exp_64 <- read.table(filename,fileEncoding="GBK", sep=",", header=TRUE)

filename <- "stefan_wcst_long_health_fixed_64.csv"
stefan_wcst_long_health_64 <- read.table(filename,fileEncoding="GBK", sep=",", header=TRUE)



check_columns <- function(wcst_long){
...
check_columns(stefan_wcst_long_exp_64)
check_columns(stefan_wcst_long_health_64)
...
stefan_wcst_long_exp_64 <- stefan_wcst_long_exp_64 %>% by(
  stefan_wcst_long_exp_64$subid,
  trial_index_calc
) %>% (function(frames){data.table::rbindlist(frames,use.names=TRUE)})()
write.csv(stefan_wcst_long_exp_64,file.path(path_persistance,'stefan_wcst_long_exp_64.csv'),sep=",")


# the new wcst_long after adding the mediate indices:
stefan_wcst_long_health_64 <- stefan_wcst_long_health_64 %>% by(
  stefan_wcst_long_health_64$subid,
  trial_index_calc
) %>% (function(frames){data.table::rbindlist(frames,use.names=TRUE)})()
write.csv(stefan_wcst_long_health_64,file.path(path_persistance,'stefan_wcst_long_health_64.csv'),sep=",")
...
stefan_wcst_short_exp <- 
  by(
    stefan_wcst_long_exp_64,
    stefan_wcst_long_exp_64$subid,
    calc_indices
  ) %>% (function(frames){data.table::rbindlist(frames,use.names=TRUE)})()
# save to file
write.csv(stefan_wcst_short_exp,file.path(path_persistance,'stefan_wcst_short_exp_64.csv'),sep=",")

# the new wcst_short after adding the mediate indices:
stefan_wcst_short_health <- 
  by(
    stefan_wcst_long_health_64,
    stefan_wcst_long_health_64$subid,
    calc_indices
  ) %>% (function(frames){data.table::rbindlist(frames,use.names=TRUE)})()
# save to file
write.csv(stefan_wcst_short_health,file.path(path_persistance,'stefan_wcst_short_health_64.csv'),sep=",")
...
multi_calc_frame_stefan_exp <- multi_summary(data=stefan_wcst_long_exp_64,framed_params=multi_cal_params)
# save to file, check to see what has been saved.
write.csv(multi_calc_frame_stefan_exp %>% round(4),file.path(path_persistance,file="multi_calc_frame_stefan_exp_64.csv"))



# deannotate to enable the split-half calculation, you need to know that this may cost most of the CPU source and may cost much time to complete.
multi_calc_frame_stefan_health <- multi_summary(data=stefan_wcst_long_health_64,framed_params=multi_cal_params)
# save to file, check to see what has been saved.
write.csv(multi_calc_frame_stefan_health %>% round(4),file.path(path_persistance,file="multi_calc_frame_stefan_health_64.csv"))
